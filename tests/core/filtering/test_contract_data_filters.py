...
def array_values(draw):
    ...
    non_matching = draw(
        st.lists(elements=st.binary(min_size=2, max_size=2)).filter(
            lambda x: x != matching
        )
    )
...
def test_data_filters_with_dynamic_arguments(
    w3, wait_for_transaction, create_filter, emitter, api_style, vals
):
    if api_style == "build_filter":
        ...
        event_filter = create_filter(
            emitter, ["LogDynamicArgs", {"filter": {"arg1": vals["matching"]}}]
        )
...
def test_data_filters_with_list_arguments(
    ...
    if api_style == "build_filter":
        ...
        txn_hashes.append(
            emitter.functions.logListArgs(arg0=non_matching, arg1=matching).transact(
                {"maxFeePerGas": 10**9, "maxPriorityFeePerGas": 10**9}
            )
        )
        txn_hashes.append(
            emitter.functions.logListArgs(arg0=matching, arg1=non_matching).transact(
                {"maxFeePerGas": 10**9, "maxPriorityFeePerGas": 10**9}
            )
        )
